...
GitHub 公開來源只包含可重現建置所需的原始碼、通用腳本、圖示及授權文件；本機發布憑證、個人化設定與內部發布操作均不放入 Repository。

## 發布前檢查

- [ ] README、授權文件與第三方通知內容一致。
- [ ] `.env*`、`build/bin/`、`dist/`、`frontend/dist/`、安裝包與本機發布資產未加入 Git。
- [ ] 原始碼不存在憑證、密碼、Token 或個人絕對路徑。
...
簽署、封裝與其他發布服務均應在私有發布環境中執行；相關工具、設定與憑證不屬於公開 Repository 的一部分。
...
- 列出正式下載檔名。
- 僅公開必要的使用者資訊，避免揭露內部發布工具、憑證設定或驗證細節。
