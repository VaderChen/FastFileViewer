...
const targetPlatforms = ["darwin/arm64"];
...
function collectNpmPackages() {
  const nodeModulesRoot = realpathSync(join(frontendRoot, "node_modules"));
  const npmProjectRoot = dirname(nodeModulesRoot);
  const output = run("npm", ["ls", "--omit=dev", "--all", "--parseable"], { cwd: npmProjectRoot });
  const packages = new Map();

  for (const directory of output.split("\n").filter(Boolean)) {
    const packageJsonPath = join(directory, "package.json");
    if (resolve(directory) === resolve(npmProjectRoot) || !existsSync(packageJsonPath)) {
      continue;
    }
